...
class ModelModuleNewsletters extends Model {
	...
	public function subscribes($data) {
		$res = $this->db->query("select * from ". DB_PREFIX ."newsletter where news_email='".$data['txtemail']."'");
		if($res->num_rows == 1)
		{
		...
		else
		{
			...
			if($this->db->query("INSERT INTO " . DB_PREFIX . "newsletter(news_email,subscribe_date,news_name) values ('".$data['txtemail']."','".date("Y-m-d")."','".$data['txtname']."')"))
			{
